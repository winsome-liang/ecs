...
class ucenter extends integrate
{
    ...
    {
    	//原方法为：   function add_user($username, $password, $email)，为了屏蔽错误，增加参数$gender = -1, $bday = 0, $reg_date=0, $md5password='' 。
        /* 检测用户名 */
    ...
    {
    	//原方法为  ：function set_cookie($username='') ，为了屏蔽错误（重写父类方法），把参数$remember增加。
        if (empty($username))
        {
            /* 摧毁cookie */
            $time = time() - 3600;
