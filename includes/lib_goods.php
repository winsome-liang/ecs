...
if(!function_exists("get_my_cat_goods")){
	function get_my_cat_goods($type = '',$cat_id, $num = 0)
	{
		$children = get_children($cat_id);
...
if(!function_exists("get_my_cat_info")){
    function get_my_cat_info($cat_id)
    {
        /* 分类信息 */
        $sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
        $cat['name'] = $GLOBALS['db']->getOne($sql);
...
if(!function_exists("get_my_promotion_goods")){
	function get_my_promotion_goods($type = '',$cat_id, $num = 0)
	{
